package com.dephoegon.delbase.aid.block.colorshift.fence;
import com.dephoegon.delbase.aid.block.stock.fenceGateBlock;

public class chiseledSandStoneFenceGate extends fenceGateBlock {
    public chiseledSandStoneFenceGate(Properties properties, String normToolTip, String shiftToolTip, String ctrlToolTip) {
        super(properties, normToolTip, shiftToolTip, ctrlToolTip, false, 0, 0, null);
    }
    public chiseledSandStoneFenceGate(Properties properties) {
        super(properties, "", "", "", false, 0, 0, null);
    }
}
